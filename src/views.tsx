...
import { event, levels, displayDeadline } from "./event.js";
...
function AwardBadge({ motif }: { motif: "scout" | "animal" | "art" }) {
  return <svg class="award-badge" viewBox="0 0 160 180" aria-hidden="true" focusable="false">
    ...
    {motif === "scout" && <g class="badge-symbol">
      <path d="M80 91C42 102 44 58 65 64 51 32 108 32 95 64 118 56 119 103 80 91Z" fill="currentColor" />
      <path d="M80 80v25m-9 0h18" fill="none" stroke="currentColor" stroke-width="4" stroke-linecap="round" />
      <path d="m80 58 3 8 9 1-7 5 2 9-7-5-7 5 2-9-7-5 9-1Z" class="badge-highlight" />
    </g>}
...
export function Layout({ title, page, children }: { title: string; page: string; children: Child }) {
  return <html lang="en"><head>
    ...
    <meta name="htmx-config" content={JSON.stringify({ historyCacheSize: 0, allowEval: false, allowScriptTags: false, selfRequestsOnly: true, includeIndicatorStyles: false })} />
    <link rel="icon" href="/images/derby-logo.png" type="image/png" /><link rel="stylesheet" href="/styles.css" />
    <script src="/htmx.min.js" defer></script><script src="/site.js" defer></script>
  </head><body hx-history="false">
    ...
    <footer><div><strong>Ankeny Girl Scouts</strong><p>Contact <a href={"mailto:" + event.contacts[0].email}>Benjamin</a> or <a href={"mailto:" + event.contacts[1].email}>Todd</a>.</p></div><a class="organizer-link" href="/admin">Organizer sign in</a></footer>
  ...
  return <Layout title="Ankeny Girl Scouts" page="home">
    <section class="hero" aria-labelledby="event-title">
      <div class="hero-copy">
        ...
        <div class="deadline" hx-get="/api/status" hx-trigger="load" hx-swap="innerHTML">Registration closes {displayDeadline(event.closesAt)}.</div>
      </div>
    ...
    <section id="race-day" class="race-day" aria-labelledby="race-day-title">
      ...
      <dl class="race-day-details">
        <div><dt>Location</dt><dd><strong>{event.venue}</strong><span>{event.address}</span></dd></div>
        <div><dt>Schedule</dt><dd><strong>{event.raceDate}</strong><span>{event.raceTimes}</span></dd></div>
        <div><dt>Check-in</dt><dd>Save your race number for check-in.</dd></div>
      </dl>
    </section>
...
function EventPhoto({ name, alt }: { name: string; alt: string }) {
  return <a class="gallery-photo" href={"/images/" + name + ".jpg"} data-gallery-photo aria-label={"View photo: " + alt} aria-haspopup="dialog"><img class="event-photo" src={"/images/" + name + "-small.jpg"} width="700" height="525" loading="lazy" decoding="async" alt={alt} /></a>;
}
export function RegisterPage() {
  return <Layout title="Register" page="register"><section class="registration-page">
    <header class="page-heading"><h1>Register a racer</h1></header>
    <div id="registration-panel" class="paper" aria-label="Registration" hx-get="/api/registration-form" hx-trigger="load" hx-swap="innerHTML"><p class="loading" role="status">Loading</p><noscript>Please enable JavaScript to register.</noscript></div>
  </section></Layout>;
}
export function GuidePage() {
  const toddEmail = "mailto:" + event.contacts[1].email;
  return <Layout title="Car guide" page="guide"><article class="guide-page">
    <header class="page-heading"><h1>Car guide</h1><p>Pick up your kit, build your car, and get ready to race.</p></header>
    <section id="kit-pickup" class="guide-pickup" aria-labelledby="pickup-title">
      <div>
    ...
    <section id="car-rules" class="guide-section" aria-labelledby="rules-title">
      ...
      <div class="guide-rules-grid">
        <div><h3>Car specifications</h3><p class="guide-note">Most measurements match the kit’s original block of wood.</p>
    ...
    <section id="building" class="guide-section" aria-labelledby="building-title">
...
export function AdminPage() {
  return <Layout title="Organizers" page="admin"><section class="admin-page"><div class="page-heading section-heading"><div><h1>Race roster</h1></div><a class="text-link" href="/.auth/logout?post_logout_redirect_uri=/">Sign out</a></div><div id="admin-content" hx-get="/api/admin/dashboard" hx-trigger="load"><p class="loading" role="status">Loading registrations</p></div></section></Layout>;
}
...
export function RegistrationForm({ csrf, submissionId, values, errors, organizer = false }: FormProps) {
  ...
  return <form action={action} method="post" hx-post={action} hx-target="this" hx-swap="outerHTML" hx-disabled-elt="find button[type='submit']">
    ...
    {organizer && <a class="text-link" href="/admin">Back to roster</a>}
  </form>;
}
export function Receipt({ row }: { row: Registration }) {
  return <>
    <div class="receipt-heading"><span class="small-tag">PINEWOOD DERBY 2027</span></div>
    <h1>Registration complete</h1>
    <p class="receipt-label">YOUR RACE NUMBER</p>
    <div class="race-number">{String(row.raceNumber).padStart(2, "0")}</div>
